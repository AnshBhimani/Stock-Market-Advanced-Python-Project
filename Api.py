...
def create_stock_file(stock_code, csv_dir, file_count):
    try:
        ...
        if not os.path.exists(csv_filename):
            ...
            file_count[0] += 1  # Increment the file count
        else:
...
def fetch_stock_data(stock_code, csv_dir, bse, file_count):
    try:
        stock_info = bse.getQuote(stock_code)
        # Create a separate CSV file for each stock if it doesn't exist
        create_stock_file(stock_code, csv_dir, file_count)
...
if __name__ == "__main__":
    ...
    file_count = [0]  # Create a list to store the file count

    try:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for stock_code, stock_name in bse.getScripCodes().items():
                executor.submit(fetch_stock_data, stock_code, csv_dir, bse, file_count)

        logger.info("Stock data updated for all stocks in the directory: %s", csv_dir)
        print(f"Total files created: {file_count[0]}")
        logger.info(f"Total files created: {file_count[0]}")
